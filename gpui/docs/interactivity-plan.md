...
### 1. Replace URL text div with real `Input` widget
...
- [ ] Import `gpui_component::input::Input`
...
### 2. Wire sub-tab click handlers (Params / Headers / Body)
...
- [ ] Add `.on_mouse_down(MouseButton::Left, ...)` to each sub-tab div calling `switch_sub_tab(tab, cx)`
...
### 3. Replace inline params/headers stubs with `KeyValueEditor` entity
...
- [ ] Add `Entity<KeyValueEditor>` fields to `RequestEditor` struct:
      ```rust
      params_editor: Option<Entity<KeyValueEditor>>,
      headers_editor: Option<Entity<KeyValueEditor>>,
      ```
- [ ] Initialize them as `None` in `RequestEditor::new()`
- [ ] When rendering, create/update editors from active tab's params/headers data
- [ ] Replace the inline params div (lines 869-897) with `self.params_editor.clone()`
- [ ] Replace the inline headers div (lines 900-928) with `self.headers_editor.clone()`
...
### 4. Replace body text div with real `Input` widget
...
- [ ] Replace the body `div().child(display_body)` (lines 944-979) with a real `Input`:
      ```rust
      ...
      ```
- [ ] Remove the manual `body_content` / `display_body` / `content_div` logic

### 5. Add a "New Collection" button to sidebar empty state
...
- [ ] In `SidebarPanel::render()` (line 538), detect `collections.is_empty()`
- [ ] Show an empty state with a "New Collection" button
- [ ] Wire button to create a default collection via `app_state.core`:
      ```rust
...
### 7. Sync method dropdown selection back to `TabState`
...
- [ ] In `select_method_by_index()`, after setting `self.selected_method`, also update AppState:
      ```rust
...
- [ ] `cargo check -p reqforge-app` — compiles cleanly
- [ ] `cargo test -p reqforge-app` — all tests pass
