...
use gpui::{div, px, App, AppContext, Context, Entity, InteractiveElement, IntoElement, MouseButton, ParentElement, Render, Styled, Subscription, Window};
...
impl Render for RequestEditor {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let url_input = if let Some(tab) = self.app_state.read(cx).active_tab() {
            // Clone the entity for use in the closure
            let url_input_entity = tab.url_input.clone();
            div()
                .w_full()
                .child(
                    Input::new(&url_input_entity)
                        .appearance(true)
                        .bordered(true)
                        .w_full()
                )
        } else {
            // No active tab - show placeholder div
            div()
                .w_full()
                .h(px(32.0))
                .px_3()
                ...
                .child(
                    div()
                        .text_sm()
                        .text_color(cx.theme().muted_foreground)
                        .child("Enter URL or right-click collection to create a request")
                )
        };
        ...
        let body_input = if let Some(input_state) = body_input_state {
            div()
                .flex_1()
                .min_h(px(200.0))
                .child(Input::new(&input_state).h_full())
        } else {
            div()
                .flex_1()
                .min_h(px(200.0))
